...
class BalancedMultiDatasetPairs(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx: int):
        ...
        e = self.entries[d]
        n = e.E_train.shape[0]
        # Derive a deterministic RNG from (seed, idx) with 64-bit wraparound
        # without triggering NumPy overflow warnings.
        A = 6364136223846793005
        B = 1442695040888963407
        mask = (1 << 64) - 1
        mixed = ((int(self.seed) * A) + (int(idx) * B) + int(d)) & mask
...
def main():
    ...
    scheduler = CosineAnnealingLR(opt, T_max=args.epochs - 1)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # LR schedule step (cosine with 1-epoch warmup)
        if epoch > 1:
            scheduler.step()
